...
import { configManager, hawtio, useHawtconfig, usePlugins } from '@hawtiosrc/core'
...
export const HawtioPage: React.FunctionComponent = () => {
  ...
  let defaultPage = defaultPlugin ? <Redirect to={{ pathname: hawtio.fullPath(defaultPlugin.path!), search }} /> : <HawtioHome />
  ...
  const sideBarShown = configManager.isSideBarShown(hawtconfig)
        ...
        <PluginNodeSelectionContext.Provider value={{ selectedNode, setSelectedNode }}>
          <Switch>
            ...
            {plugins
              ...
              .map(plugin => (
                <Route key={plugin.id} path={`${hawtio.fullPath(plugin.path!, WILDCARD)}`}>{React.createElement(plugin.component!)}</Route>
              ))}
            <Route key={HELP} path={hawtio.fullPath(HELP, WILDCARD)}><HawtioHelp /></Route>
            <Route key={PREFERENCES} path={hawtio.fullPath(PREFERENCES, WILDCARD)}><HawtioPreferences /></Route>
            <Route key={INDEX_KEY} path={hawtio.fullPath('index.html')}><Redirect to={hawtio.fullPath(INDEX)} /></Route>
            <Route key={ROOT} exact path={`${hawtio.fullPath(INDEX)}`}>{defaultPage}</Route>
          </Switch>
